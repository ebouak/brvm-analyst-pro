'use client';

import { useState } from 'react';
import { addPosition } from '@/app/portefeuille/actions';

interface Props {
  isOpen: boolean;
  onClose: () => void;
  instruments: { code: string; designation: string | null }[];
  watchlistId: string | null;
}

export default function NewPositionModal({ isOpen, onClose, instruments, watchlistId }: Props) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [rating, setRating] = useState(0);

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="bg-surface border border-border rounded-xl shadow-lg max-w-md w-full mx-4">
        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
          <h2 className="text-lg font-semibold">Nouvelle position</h2>
          <button type="button" onClick={onClose} className="text-muted hover:text-fg text-lg" aria-label="Fermer">✕</button>
        </div>

        <form
          action={addPosition}
          onSubmit={() => { setIsSubmitting(true); setTimeout(onClose, 100); }}
          className="p-6 space-y-4"
        >
          {watchlistId && <input type="hidden" name="watchlist_id" value={watchlistId} />}
          <input type="hidden" name="rating" value={String(rating)} />

          <div>
            <label htmlFor="pm-code" className="block text-sm font-medium mb-1">Code *</label>
            <select
              id="pm-code"
              name="code"
              required
              className="w-full bg-bg border border-border rounded px-3 py-2 text-sm"
            >
              <option value="">— Sélectionner un titre —</option>
              {instruments.map((ins) => (
                <option key={ins.code} value={ins.code}>
                  {ins.code}{ins.designation ? ` — ${ins.designation}` : ''}
                </option>
              ))}
            </select>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="pm-quantite" className="block text-sm font-medium mb-1">Quantité *</label>
              <input id="pm-quantite" name="quantite" type="number" placeholder="0" required step="any"
                className="w-full bg-bg border border-border rounded px-3 py-2 text-sm" />
            </div>
            <div>
              <label htmlFor="pm-prix" className="block text-sm font-medium mb-1">Prix d'entrée *</label>
              <input id="pm-prix" name="prix_entree" type="number" placeholder="0" required step="any"
                className="w-full bg-bg border border-border rounded px-3 py-2 text-sm" />
            </div>
          </div>

          <div>
            <label htmlFor="pm-date" className="block text-sm font-medium mb-1">Date d'entrée</label>
            <input id="pm-date" name="date_entree" type="date"
              className="w-full bg-bg border border-border rounded px-3 py-2 text-sm" />
          </div>

          <div>
            <label htmlFor="pm-note" className="block text-sm font-medium mb-1">Note</label>
            <textarea id="pm-note" name="note" placeholder="Notes personnelles…" rows={3}
              className="w-full bg-bg border border-border rounded px-3 py-2 text-sm resize-none" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-2">Note personnelle</label>
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5].map((star) => (
                <button key={star} type="button" onClick={() => setRating(star)}
                  className={`text-2xl transition ${rating >= star ? 'text-yellow-400' : 'text-muted'}`}
                  aria-label={`Note ${star} étoile(s)`}>★</button>
              ))}
            </div>
          </div>

          <div className="flex gap-3 pt-4">
            <button type="button" onClick={onClose} disabled={isSubmitting}
              className="flex-1 px-4 py-2 rounded border border-border text-sm font-medium hover:bg-bg/40 transition">
              Annuler
            </button>
            <button type="submit" disabled={isSubmitting}
              className="flex-1 px-4 py-2 rounded bg-up/90 hover:bg-up text-black text-sm font-medium transition disabled:opacity-50">
              {isSubmitting ? 'Ajout…' : 'Ajouter'}
            </button>
          </div>
        </form>
      </div>
    </div>
  );
}
