// frontend/lib/pairingCodes.ts
import { randomInt } from 'node:crypto';

/**
 * Codes d'appairage à usage unique, communs à tous les canaux (WhatsApp,
 * Telegram).
 *
 * Ce module vivait dans lib/whatsappAgent/pairing.ts, qui n'a jamais rien eu
 * de spécifique à WhatsApp : un code aléatoire, une durée de vie, une
 * reconnaissance de forme. Le sortir de là évite qu'un canal en importe un
 * autre pour une fonction purement générique. `whatsappAgent/pairing.ts`
 * réexporte ce fichier : aucun appelant existant ne change.
 *
 * PUR : aucune I/O, aucun accès base. Testé par whatsappAgent/pairing.test.mjs.
 */

// Alphabet sans caractères ambigus (O/0, I/1) : le code est lu à l'écran puis
// retapé à la main dans l'application de messagerie.
...
/** Horodatage d'expiration à stocker dans la colonne `expires_at`. */
...
/**
 * Un message entrant est-il un code d'appairage plutôt qu'une question à
 * l'agent ? Testé AVANT le traitement conversationnel normal.
 *
 * Les deux canaux partagent le même format, sans risque de confusion : chacun
 * interroge sa propre table, donc un code WhatsApp envoyé à Telegram sera
 * simplement « inconnu ».
 */
